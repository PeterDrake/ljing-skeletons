...
public class Mazes {
	 ...
	 */
	public static final int[][] OFFSETS = { { 0, 1 }, { 1, 0 }, { 0, -1 },
			{ -1, 0 } };
	...
	/**
	 * Returns a new array of locations containing start followed by all of the locations in restOfPath.
	 */
	public static int[][] addToFront(int[] start, int[][] restOfPath) {
	...
	public static void drawMaze(boolean[][][] passages) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < width; y++) {
				if (passages[x][y][NORTH]
						|| (y + 1 < width && passages[x][y + 1][SOUTH])) {
					StdDraw.line(x, y, x, y + 1);
				}
				if (passages[x][y][EAST]
						|| (x + 1 < width && passages[x + 1][y][WEST])) {
	 ...
	 *         otherwise.
	 */
	public static int[] expandLocation(boolean[][][] passages,
			int[][] unexplored, int n, int[] here, int direction) {
		// TODO You have to write this, replacing the line below
		return null;
	}
	 ...
	 * @param frontier
	 *            a list of locations that have been reached by some edge.
	 * @param unexplored
	 *            a list of locations that have not yet been reached.
	 * @param counts
	 ...
	 */
	public static int[] expandMaze(boolean[][][] passages, int[][] done,
			int[][] frontier, int[][] unexplored, int[] counts,
			int[] lastExploredLocation) {
		int[] here;
		...
		for (int i = 0; i < 4; i++) {
			int[] there = expandLocation(passages, unexplored, counts[2], here,
					direction);
			if (there != null) {
	...
	public static void main(String[] args) {
		...
		while (counts[2] > 0) {
			lastExploredLocation = expandMaze(passages, done, frontier,
					unexplored, counts, lastExploredLocation);
			drawMaze(passages);
			StdDraw.show(25);
		}
		// Solve the maze
		int[][] solution = solve(passages, new int[] { 0, 0 }, new int[] {
				width - 1, width - 1 });
